/* ───────────────────────────────────────
   4. 원탁토론 — 참여자(모바일) 화면. 토론모드(대기) ↔ 발표모드(업로드 + 갤러리 + 하트).

   고칠 때 ─ 작성양식 안내 문구           → content/04-board.json
             업로드·투표 로직             → 이 파일
             리사이즈 크기                → js/storage.js
   쓰는 것 ─ js/db.js(board/state, boardPhotos, 하트) · js/storage.js(사진 업로드)

   신원 ─ 사진은 조 번호(ctx.team)로 묶인다. 하트는 "각자"이므로 조 번호와
   무관한 voterId(기기별 uuid, localStorage)로 누가 눌렀는지 구분한다.
   ─────────────────────────────────────── */
import { watch, watchCollection, toggleHeart, saveMainPhoto, addExtraPhoto, path } from '../../db.js';
...
export default {
  ...
  mount(ctx) {
    ...
    let data = null, mode = 'write', photos = {}, uploading = false;
    const unsubs = [];
    ...
    function render() {
      if (!data) { ctx.root.innerHTML = `<div class="center"><div class="pulse"></div></div>`; return; }
    ...
    function viewWrite() {
      ...
      return `<div class="center">
        <div class="big">지금은 종이에 작성 중이에요</div>
        <div class="sub">"${esc(p.lines[0])} ___ 입니다"<br>테이블에서 의견을 나눠 종이에 적어주세요</div>
      </div>`;
    }
    ...
    function viewPresent() {
      ...
      const uploadBox = `
        <div class="upload" id="up1">
          ...
          <div class="ic">📷</div>
          <div class="t">${mine[0] ? '다시 찍어서 교체하기' : '작성한 종이를 촬영해서 올려주세요'}</div>
        </div>
      ...
      return `<div class="h">우리 조 작성물 올리기</div>${uploadBox}
        <div class="h" style="margin-top:28px">발표 갤러리 · 마음에 드는 정책에 하트를 눌러주세요</div>
